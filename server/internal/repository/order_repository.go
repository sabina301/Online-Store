...
func (r *OrderRepository) GetAllOrdersUser(userId int) ([]entity.Order, error) {
	tx, err := r.db.Begin()
	if err != nil {
		tx.Rollback()
		return nil, err
	...
	query = "SELECT * FROM order_from_cart WHERE cart_id = $1"
	rows, err := tx.Query(query, cartId)
	if err != nil {
		tx.Rollback()
		return nil, err
	...
	for rows.Next() {
		...
		err := rows.Scan(&order.Id, &order.CartId)
		if err != nil {
			tx.Rollback()
			return nil, err
		}
...
func (r *OrderRepository) GetAllOrdersAdmin() ([]entity.Order, error) {
	tx, err := r.db.Begin()
	if err != nil {
		tx.Rollback()
		return nil, err
	}
	query := "SELECT * FROM order_from_cart"
